...
if not os.path.exists("questions.py"):
    print("You're missing the questions.py file!\nDid you download it correctly?\n")
    exit(0)
#
# try:
...
assert Question
#except:
#     print("Something went wrong with the questions.py file!\n\
# Did you download it correctly?\n")
#     exit(0)
...
# number of seconds before timer times out.
# double the time if we have to check for downloaded files
TIME = 60 if not "FILES_TO_DOWNLOAD" in globals() or FILES_TO_DOWNLOAD == [] else 120
...
def get_expected_json():
    global expected_json #defined in questions.py
...
def get_python_cmd():
    ...
    for cmd in cmds:
        try:
            out = subprocess.check_output(cmd + ' -V', shell=True, universal_newlines=True, stderr=subprocess.DEVNULL)
...
def inject_safe_join():
    safe_join_code = "\
    if not 'joined_paths' in globals():\
        joined_paths = []\
        ...
        def open_safe(*args, **kwargs):\
            ...
            for path in joined_paths:\
                if path is filepath:\
                    ...
                    break\
            if not have_seen:\
                raise IOError('You tried to open a file without making its path using join.') \
\
            return old_open(*args, **kwargs)\
        open = open_safe"
    safe_join_code.replace('\n','\\n').replace("\"", "\\\"").replace("'", "\\'")
...
def rerun_notebook(orig_notebook):
    ...
    try:
        ...
        if py_cmd:
            cmd = py_cmd + ' -m ' + cmd
        with open("error_out.txt", "wb") as err_fp:
            subprocess.check_output(cmd, shell=True, stderr=err_fp)
    except Exception as e:
        with open("error_out.txt", "r") as err_fp:
            lines = str(err_fp.read())
            if "FILES_TO_DOWNLOAD" in globals() and FILES_TO_DOWNLOAD != []:
                #print(lines)
                if "ValueError: Expected object or value" in lines or "FileNotFoundError" in lines:
                    for file in FILES_TO_DOWNLOAD:
                        if file in lines:
                            print("You have an error in your notebook. It probably pertains to downloading " + file)
                            err_fp.close()
                            if os.path.exists("error_out.txt"):
                                os.remove("error_out.txt")
                            exit()
            print(lines)
        if os.path.exists("error_out.txt"):
            os.remove("error_out.txt")
...
# FILES_TO_DOWNLOAD defined in questions.py
def record_download_timestamps(timestamps):
    for file in FILES_TO_DOWNLOAD:
        timestamps[file] = os.path.getmtime(file)

def verify_cached_download(timestamps):
    for file in FILES_TO_DOWNLOAD:
        if os.path.getmtime(file) != timestamps[file]:
            return False
    return True

def get_text_actual(cell, expected, format):
    outputs = cell.get('outputs', [])
    if len(outputs) == 0:
        return (False, 'no Out[N] output found for cell')
    actual_lines = None
    for out in outputs:
    ...
    if actual_lines == None:
        return (False, 'no Out[N] output found for cell (note: printing the output does not work)')
    ...
    if 'namedtuple' in format:
        try:
            ...
            return (False, "expected to find namedtuple object of type %s but found type %s" % (type(expected).__name__, obfuscate3))
        except TypeError:
            return (False, "expected to find attributes: %s in namedtuple %s" % (str(expected._fields), type(expected).__name__))
    else:
        if expected == actual:
            return (True, actual)
        try:
            ...
            return (False, "CANNOT PARSE CELL!")
        except IndentationError:
            return (False, "expected to find type %s but found a DataFrame" %(type(expected).__name__))
    return (True, actual)


def check_cell_text(qnum, cell, format):
    expected = expected_json[str(qnum)]
    actual_data = get_text_actual(cell, expected, format)
    if actual_data[0] == False:
...
def check_cell_html(qnum, cell):
    outputs = cell.get('outputs', [])
    if len(outputs) == 0:
        return 'no Out[N] output found for cell'
...
def check_cell(question, cell, visible=False):
    if visible:
...
def grade_answers(cells, visible=False):
...
def verify_new_notebook(old_cells, new_cells, orig_notebook):
    ...
    for t in new_nb_grading["tests"]:
        if t['test'] not in old_new_nb_match:
            msg = "\nQuestion %s has no output in %s, but has output when notebook is rerun." % (t['test'], orig_notebook)
            if TEXT_FORMAT in formats[t['test']]:
                output = get_text_actual(new_cells[t['test']], expected_json[str(t['test'])], formats[t['test']])
                if output[0] == False:
                    msg += "\nError in cell when notebook is rerun: %s" % (output[1])
                else:
                    msg += "\nOutput in cell when notebook is rerun: %s" % repr(output[1])
            return FAIL + msg
        old_new_nb_match[t['test']].append(t['result'])
    for q in old_new_nb_match:
        if len(old_new_nb_match[q]) != 2:
            msg = "\nQuestion %s has output in %s, but has no output when notebook is rerun." % (q, orig_notebook)
            if TEXT_FORMAT in formats[q]:
                output = get_text_actual(old_cells[q], expected_json[str(q)], formats[q])
                if output[0] == False:
                    msg += "\n\nError in cell of %s: %s" % (orig_notebook, output[1])
                else:
                    msg += "\n\nOutput in cell of %s: %s" % (orig_notebook, repr(output[1]))
            return FAIL + msg
        if old_new_nb_match[q][0] != old_new_nb_match[q][1]:
            if "no Out[N] output found for cell" in old_new_nb_match[q][0]:
                msg = "\nQuestion %s has no output in %s, but has output when notebook is rerun." % (q, orig_notebook)
                if TEXT_FORMAT in formats[q]:
                    output = get_text_actual(new_cells[q], expected_json[str(q)], formats[q])
                    if output[0] == False:
                        msg += "\n\nError in cell when notebook is rerun: %s" % (output[1])
                    else:
                        msg += "\n\nOutput in cell when notebook is rerun: %s" % repr(output[1])
            elif "no Out[N] output found for cell" in old_new_nb_match[q][1]:
                msg = "\nQuestion %s has output in %s, but has no output when notebook is rerun." % (q, orig_notebook)
                if TEXT_FORMAT in formats[q]:
                    output = get_text_actual(old_cells[q], expected_json[str(q)], formats[q])
                    if output[0] == False:
                ...
                is_grading_server = "dockerGrader" in os.getcwd()
                if NO_RERUN in formats[q] and is_grading_server:
                    return PASS
                msg = "\nQuestion %s has a different output when notebook is rerun." % (q)
                if TEXT_FORMAT in formats[q]:
                    old_output = get_text_actual(old_cells[q], expected_json[str(q)], formats[q])
                    new_output = get_text_actual(new_cells[q], expected_json[str(q)], formats[q])
                    if old_output[0] == False:
                        msg += "\n\nError in cell of %s: %s" % (orig_notebook, old_output[1])
                    else:
                        msg += "\n\nOutput in cell of %s: %s" % (orig_notebook, repr(old_output[1]))
                    if new_output[0] == False:
                        msg += "\n\nError in cell when notebook is rerun: %s" % (new_output[1])
                    else:
                        msg += "\n\nOutput in cell when notebook is rerun: %s" % repr(new_output[1])
            return FAIL + msg
    return PASS
...
def main():
    ...
    # If we're checking for caching of downloaded files, run nb again
    NO_CACHE = False
    if "FILES_TO_DOWNLOAD" in globals() and FILES_TO_DOWNLOAD != []:
        print("Running notebook again to check for caching...")
        timestamps = {}
        record_download_timestamps(timestamps)
        new_nb = rerun_notebook(orig_notebook)
        NO_CACHE = not verify_cached_download(timestamps)
        if NO_CACHE:
            print("\n**Warning:** -5 for redownloading files that already exist.")
    ...
    verify_match_msg = verify_new_notebook(old_answer_cells, new_answer_cells, orig_notebook)
    if verify_match_msg != PASS:
        msg_length = max([len(line) for line in verify_match_msg.split("\n")])
        msg_length = min(msg_length, 120)
        print('\n' + '*'*msg_length)
